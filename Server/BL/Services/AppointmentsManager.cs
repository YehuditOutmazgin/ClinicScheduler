...
namespace BL.Services
{
    public class AppointmentManager : IAppointmentsManager
    {
        ...
        {
            var appointments = await _appointmentsDal.GetAppointmentsTherapistAndDate(therapistId, date);
            if(appointments == null)
            {
                return null;
            }
            return _mapper.Map<List<BLAppointment>>(appointments);
        }
        ...
        //    }
        //}
        public async Task<List<BLPastAppointment>> GetPastAppointmentsByPatientId(int patientId)
        {
            try
            {
                if (patientId < 0)
                ...
                var appointments = await _PastAppointmentsDal.GetAllPastAppointmentsByPatientId(patientId);

                if (appointments == null)
                    return null;

                return _mapper.Map<List<BLPastAppointment>>(appointments);
            }
            catch (Exception)
            {
                return null;
