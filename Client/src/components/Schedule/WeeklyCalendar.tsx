...
import { fetchAvailableAppointmentsForTherapistWeek, scheduleAppointment } from "../../store/slices/appointmentSlice"
...
const WeeklyCalendar: React.FC<WeeklyCalendarProps> = ({ selectedTherapist, selectedSpecialty, patientId }) => {
  ...
  const handleAppointmentClick = (appointment: AvailableAppointment) => {
    ...
    if (role === "secretary" || role === "client") {
      setShowBookingModal(true)
    }
  }

  const handleBookAppointment = async () => {
    if (!selectedAppointment || !bookingPatientId) return
