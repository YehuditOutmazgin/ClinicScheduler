...
const AppointmentsList: React.FC = () => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "scheduled":
        return { bg: "var(--pastel-green)", color: "var(--dark-green)" }
      case "completed":
        return { bg: "var(--pastel-blue)", color: "var(--dark-blue)" }
      case "cancelled":
        return { bg: "var(--pastel-coral)", color: "var(--dark-coral)" }
      default:
        return { bg: "var(--gray)", color: "var(--dark-gray)" }
    }
  }
    ...
    <div className="container">
      ...
      <div className="card">
        ...
        {futureAppointments.length === 0 && (
          <p style={{ textAlign: "center", color: "var(--dark-gray)", padding: "20px" }}>אין תורים עתידיים</p>
        )}
      </div>
    </div>
  )
