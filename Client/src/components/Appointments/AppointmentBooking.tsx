...
const AppointmentBooking: React.FC = () => {
    ...
    if (selectedTherapist && selectedDate) {
      dispatch(
        fetchAvailableAppointmentsForTherapistWeek({
          therapistId: Number.parseInt(selectedTherapist),
  ...
  return (
    <div className="appointment-booking">
      <h2 className="page-title">קביעת תור</h2>
      ...
      <div className="booking-card">
        <div className="booking-form">
          ...
          {availableAppointments.length > 0 && (
            <div className="appointments-section">
              <h4 className="section-title">תורים זמינים</h4>
              <div className="appointments-grid">
                {availableAppointments.map((appointment) => (
                  <button
                    key={appointment.appointmentId}
                    onClick={() => setSelectedAppointment(appointment.appointmentId)}
                    className={`appointment-slot ${
                      selectedAppointment === appointment.appointmentId ? "selected" : "available"
                    }`}
                  >
                    ...
                    <div className="slot-duration">{appointment.durationMinutes} דק'</div>
                  </button>
                ))}
              </div>
            </div>
        ...
        {selectedAppointment && (
          <div className="booking-actions">
            <button onClick={handleBookAppointment} className="btn btn-primary booking-btn">
              קביעת התור
            </button>
          </div>
        )}
