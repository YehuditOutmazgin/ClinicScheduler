...
import { fetchFutureAppointmentsByPatient, fetchPatientHistory, fetchCanceledAppointments, } from "../../redux/slices/appointmentSlice"
...
const PatientManagement: React.FC = () => {
  ...
  const { appointments, pastAppointments, canceledAppointments, loading: appointmentLoading, } = useSelector((state: RootState) => state.appointments)
  ...
  useEffect(() => { if (patient) { setEditedPatient(patient) } }, [patient])
  ...
  const handleScheduleAppointment = () => { navigate(`/schedule?patientId=${patientId}`) }
  const formatDate = (dateString: string) => { return new Date(dateString).toLocaleDateString("he-IL") }
  const formatTime = (dateString: string) => { return new Date(dateString).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit", }) }
  ...
  return (<div className="container patient-management">
      ...
      {isEditing ? (<div>
        <div className="patient-details">
          <div className="patient-detail">
            ...
            <input type="text" className="form-input" value={editedPatient?.firstName || ""} onChange={(e) => setEditedPatient((prev) => (prev ? { ...prev, firstName: e.target.value } : null))} />
          </div>
          <div className="patient-detail">
            <label className="detail-label">שם משפחה</label>
            <input type="text" className="form-input" value={editedPatient?.lastName || ""} onChange={(e) => setEditedPatient((prev) => (prev ? { ...prev, lastName: e.target.value } : null))} />
          </div>
          <div className="patient-detail">
            <label className="detail-label">תאריך לידה</label>
            <input type="date" className="form-input" value={editedPatient?.birthDate || ""} onChange={(e) => setEditedPatient((prev) => (prev ? { ...prev, birthDate: e.target.value } : null))} />
          </div>
          <div className="patient-detail">
            <label className="detail-label">טלפון</label>
            <input type="text" className="form-input" value={editedPatient?.phoneNumber || ""} onChange={(e) => setEditedPatient((prev) => (prev ? { ...prev, phoneNumber: e.target.value } : null))} />
          </div>
        </div>
    ...
    <div className="appointments-section">
      ...
      <div className="appointment-tabs">
        <button className={`tab-button ${activeTab === "future" ? "active" : ""}`} onClick={() => setActiveTab("future")}
        >
        ...
        <button className={`tab-button ${activeTab === "canceled" ? "active" : ""}`} onClick={() => setActiveTab("canceled")}
        >
      ...
      <div className="appointment-list">
        {activeTab === "future" && (<>
          {appointments.length > 0 ? (appointments.map((appointment) => (<div key={appointment.appointmentId} className="appointment-item">
        ...
        {activeTab === "past" && (<>
          {pastAppointments.length > 0 ? (pastAppointments.map((appointment) => (<div key={appointment.appointmentId} className="appointment-item">
            <div className="appointment-header">
              <div className="appointment-date-time">
                {formatDate(appointment.appointmentDate)} בשעה {formatTime(appointment.appointmentDate)}
              </div>
        ...
        {activeTab === "canceled" && (<>
          {canceledAppointments.length > 0 ? (canceledAppointments.map((appointment) => (<div key={appointment.appointmentId} className="appointment-item">
            <div className="appointment-header">
              <div className="appointment-date-time">
                {formatDate(appointment.appointmentDate)} בשעה {formatTime(appointment.appointmentDate)}
              </div>
